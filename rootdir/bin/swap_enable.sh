...
function configure_linhai_parameters() {
...
function configure_memory_parameters() {
        ...
        case $prjname in
            ...
            "21121"|"21301")
                if [ $MemTotal -gt 8388608 ]; then
                    echo 64 > /proc/boost_pool/mtk_mm-uncached/min
                    echo 128 > /proc/boost_pool/mtk_mm/min
                    echo 11 > /proc/sys/vm/watermark_scale_factor
                else
                    echo 11 > /proc/sys/vm/watermark_scale_factor
                fi
                ;;
            "21641"|"21642"|"21649"|"216BE"|"216BF"|"216E5"|"216E6")
                if [ $MemTotal -le 8388608 ]; then
                    echo 25 > /proc/sys/vm/watermark_scale_factor
                else
                    echo 16 > /proc/sys/vm/watermark_scale_factor
	...
	kcompactd_pid=`cat /sys/module/oplus_bsp_zram_opt/parameters/kcompactd_pid`
	if [ ! -z "$kcompactd_pid" ]; then
		if [ ${kcompactd_pid} -gt 0 ]; then
			echo $kcompactd_pid > /dev/cpuset/kswapd-like/tasks
		fi
	fi
}
